...
| 10 | [Ring on Android + the musl allocator](musl-allocator-android/NOTE.md) | Ring Google Group | no | **DRAFTED, NOT SENT** — Ring runs on Android unmodified (no NDK, no Qt), and one linked allocator makes string-heavy code 3.6–7.7× faster on any static musl build, Alpine included. Evidence: FINDINGS F-40. Mansour posts it. |
| 11 | [breath_first_search: the error handler that crashes](samples-defects/NOTE.md) | Ring Google Group | no | **DRAFTED, NOT SENT** — `exit(1)` meant as C-style terminate, but Ring's exit is loop-break: the queue-underflow branch raises R9 the day it finally runs. Found by rpp/exit-outside-loop over the 1,605 shipped samples; the only occurrence. Evidence: F-45. Mansour posts it. |
